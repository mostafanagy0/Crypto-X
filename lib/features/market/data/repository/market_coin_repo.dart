...
class MarketCoinsRepository extends BaseMarketCoinsRepository {
  ...
  @override
  Future<Either<Failure, CoinDetails>> getCoinDetails(String id) async {
    final result = await baseMarketCoinsRemoteDataSource.getCoinDetails(id);
    try {
      return Right(result);
    } on ServerException catch (failure) {
      return Left(ServerFailure(failure.errorMessageModel.statusMessage!));
  ...
  Future<Either<Failure, MarketChart>> getMarketChart({
    ...
    try {
      final result = await baseMarketCoinsRemoteDataSource.fetchMarketChart(
        coinId: coinId,
